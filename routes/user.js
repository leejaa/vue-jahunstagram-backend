...
router.get('/', isLoggedIn , (req, res) => { // /api/user/
  ...
  console.log('api/user enter...');

  console.log(`user : ${JSON.stringify(req.user)}`);

  if(!req.user){
    return res.status(401).send(`사용자 정보가 없습니다.`);
  }
...
router.post('/', async (req, res, next) => { // POST /api/user 회원가입
  try {
    ...
    const hashedPassword = crypto.createHash('sha512').update(req.body.password,).digest('base64');
    ...
    return req.login(user, async (loginErr) => {
      try {
        ...
        const fullUser = await db.User.findOne({
          ...
          attributes: ['id', 'nickname', 'userId', 'avatar'],
        });
